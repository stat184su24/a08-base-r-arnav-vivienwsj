...
```{r}
species_last_5 <- tail(iris$Species, 5)
species_last_5
```
...
rows <- 3
cols <- 4
Counting <- data.frame(matrix(ncol = cols, nrow = rows))
...
plotNormal <- function(repetitions, mean, sd) {
  nums <- rnorm(repetitions, mean, sd)
  hist(nums)
}
...
plotNormal <- function(repetitions, mean, sd, color = "blue") {
  nums <- rnorm(repetitions, mean, sd)
  hist(nums, main = paste("Histogram from a Normal(", mean, ",", sd, ") Distribution"), col = color)
}
